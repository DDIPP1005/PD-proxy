...
增强选项(环境变量):
  PD_SNELL_MAJOR=4|5|6  PD_SNELL_MODE=shadowtls  PD_SNELL_TLS_VERSION=v3
  PD_SNELL_V6_MODE=default|unshaped  PD_HY2_HOP=5  PD_HY2_HOP_RANGE=30000-30100
  PD_SNELL_PORT=12345  PD_HY2_PORT=23456  PD_VLESS_PORT=34567  PD_ANYTLS_PORT=45678
  PD_VLESS_DEST=...:443
  PD_SNELL_MANUAL_TIMEOUT=6  PD_SNELL_PROBE_PARALLEL=12  PD_SNELL_VERSION=v6.0.0rc
...
# Snell 选项
PD_OPT_SNELL_MAJOR="${PD_SNELL_MAJOR:-5}"                # 4 | 5 | 6
PD_OPT_SNELL_MODE="${PD_SNELL_MODE:-standard}"          # standard | shadowtls
PD_OPT_SNELL_V6_MODE="${PD_SNELL_V6_MODE:-default}"     # default | unshaped
...
if [ -z "${PD_SNELL_MAJOR:-}" ] && [ -n "${PD_SNELL_VERSION:-}" ]; then
    case "$PD_SNELL_VERSION" in
        v4.*) PD_OPT_SNELL_MAJOR=4 ;; v5.*) PD_OPT_SNELL_MAJOR=5 ;; v6.*) PD_OPT_SNELL_MAJOR=6 ;;
    esac
fi
...
has_ipv6() {
    # 不用 grep -q：在 pipefail 下，多地址输出会令上游 ip 因 SIGPIPE 失败。
    ip -6 addr show scope global 2>/dev/null | grep 'inet6 ' >/dev/null
}
...
prepare_snell_listen_family() {
    local major="$1" shadowtls_enabled="${2:-0}"
    local requested="${PD_LISTEN_FAMILY:-auto}"
    if [ "$major" != 6 ] && [ "$shadowtls_enabled" != 1 ]; then
        case "$requested" in
            auto) PD_LISTEN_FAMILY=ipv4 ;;
            ipv4) ;;
            ipv6|dual) die "普通 Snell v${major} 不支持 IPv6/dual 监听；请使用 ipv4、Snell v6，或在 ShadowTLS 外层提供双栈" ;;
            *) die "PD_LISTEN_FAMILY 仅支持 auto|ipv4|ipv6|dual" ;;
        esac
    fi
    prepare_listen_family
}
...
snell_manual_html() {
    ...
    # 英文Markdown包含官方资产链接且更新最快；中文页可能返回200但内容滞后，不能放在首位。
    curl_https -fsSL --connect-timeout 2 --max-time "$timeout" "https://kb.nssurge.com/surge-knowledge-base/release-notes/snell.md" 2>/dev/null \
        || curl_https -fsSL --connect-timeout 2 --max-time "$timeout" "https://kb.nssurge.com/surge-knowledge-base/zh/release-notes/snell.md" 2>/dev/null \
...
snell_probe_candidates() {
    ...
    # Snell v6 预发布阶段依次经历 beta / rc；优先探测 rc，再探测 beta，最后探测正式版命名。
    if [ "$major" = 6 ]; then
        local prerelease beta
        printf 'v6.0.0rc\n'
        for ((prerelease=30; prerelease>=1; prerelease--)); do
            printf 'v6.0.0rc%s\n' "$prerelease"
        done
        for ((beta=30; beta>=1; beta--)); do
            printf 'v6.0.0b%s\n' "$beta"
        done
    fi
...
snell_probe_batch() {
    ...
    for probe in "$@"; do
        (
            ...
            curl_https -fsI --retry 2 --retry-all-errors --retry-delay 1 --connect-timeout 1 --max-time 8 "$probe_url" >/dev/null 2>&1 \
                && printf '%s\n' "$probe" > "$tmpdir/${probe}"
        ) &
        pids+=("$!")
    done
...
snell_version_sort() {
    awk '
        {
            original=$0; value=$0; sub(/^v/, "", value)
            split(value, part, ".")
            patch=part[3]; suffix=""; patch_num=patch+0
            sub(/^[0-9]+/, "", patch); suffix=patch
            channel=0; prerelease=0
            if (suffix == "") {
                channel=3
            } else if (suffix == "rc") {
                channel=2
            } else if (suffix ~ /^rc[0-9]+$/) {
                channel=2; sub(/^rc/, "", suffix); prerelease=suffix+0
            } else if (suffix ~ /^b[0-9]+$/) {
                channel=1; sub(/^b/, "", suffix); prerelease=suffix+0
            }
            printf "%09d %09d %09d %d %09d %s\n", part[1]+0, part[2]+0, patch_num, channel, prerelease, original
        }
    ' | sort -k1,1nr -k2,2nr -k3,3nr -k4,4nr -k5,5nr | awk '{print $6}'
}
...
# ---- Snell v4/v5/v6 ----
snell_validate_major() {
    case "$1" in
        4|5|6) return 0 ;;
        *) err "Snell协议版本仅支持4、5、6，当前: $1"; return 1 ;;
    esac
}
...
snell_version_major() {
    ...
    major="${major%%.*}"
    snell_validate_major "$major" || return 1
    printf '%s\n' "$major"
}

snell_installed_major() {
    local major version
    major=$(state_get snell snell_major 2>/dev/null || true)
    if snell_validate_major "$major" 2>/dev/null; then
        printf '%s\n' "$major"
        return 0
    fi
    version=$(state_get snell version 2>/dev/null || true)
    if [ -n "$version" ]; then
        snell_version_major "$version" && return 0
    fi
    # 兼容旧状态：历史ShadowTLS安装固定为v4，普通Snell固定为v5。
    [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && printf '4\n' || printf '5\n'
}

snell_get_version() {
    local major="${1:-${PD_OPT_SNELL_MAJOR:-5}}" v cache_file
    snell_validate_major "$major" || return 1
    cache_file="$BASE_DIR/.snell-version-${major}-${ARCH}"
    mkdir -p "$BASE_DIR"
    if [ -n "${PD_SNELL_VERSION:-}" ]; then
        [[ "$PD_SNELL_VERSION" =~ ^v${major}\.[0-9]+\.[0-9]+[a-z0-9]*$ ]] \
            || die "PD_SNELL_VERSION与所选Snell v${major}不匹配"
        echo "$PD_SNELL_VERSION"
        return 0
    fi
    # 官方Markdown/知识库为权威来源：优先解析真实下载资产，避免“v6.0.0 Beta”标题误判为不存在的稳定版。
    local manual=""
    manual=$(snell_manual_html)
    if [ -n "$manual" ]; then
        v=$(printf '%s\n' "$manual" \
            | sed -n "s#.*snell-server-\(v${major}\.[0-9][0-9]*\.[0-9][0-9]*[a-z0-9]*\)-linux-.*#\1#p" \
            | { [ "$major" = 6 ] && cat || grep -E "^v${major}\.[0-9]+\.[0-9]+$"; } \
            | snell_version_sort | sed -n '1p' || true)
        if [ -z "$v" ]; then
            v=$(printf '%s\n' "$manual" \
                | sed -n "s/^### v\(${major}\.[0-9][0-9]*\.[0-9][0-9]*[a-z0-9]*\).*/v\1/p" \
                | { [ "$major" = 6 ] && cat || grep -E "^v${major}\.[0-9]+\.[0-9]+$"; } \
                | snell_version_sort | sed -n '1p' || true)
        fi
    fi
    ...
    if [ "${PD_STRICT_LATEST:-0}" = 1 ]; then
        err "$(snell_strict_latest_msg "$major")"
        return 1
    fi
    warn "无法快速读取Snell官方文档，改用下载源探测Snell v${major}可用版本" >&2
    v=$(snell_probe_latest "$major" || true)
    if [ -n "$v" ]; then
        printf '%s\n' "$v" | atomic_write_file "$cache_file" 600
        echo "$v"
        return 0
    fi
    if snell_cache_get "$major" "$cache_file" >/dev/null 2>&1; then
        warn "下载源探测失败，使用上次成功缓存版本" >&2
        snell_cache_get "$major" "$cache_file"
        return 0
    fi
    err "$(snell_manual_fail_msg "$major")"
    return 1
}

snell_download() {
    local ver="$1" major
    major=$(snell_version_major "$ver") || return 1
    require_version "$ver" "Snell v${major}"
    if [ "$major" = 6 ]; then
        case "$(snell_arch)" in
            amd64|i386|aarch64) ;;
            *) die "Snell v6不支持当前架构: $(snell_arch)" ;;
        esac
    fi
    local url="https://dl.nssurge.com/snell/snell-server-${ver}-linux-$(snell_arch).zip"
    step "下载 Snell v${major} $ver ..."
    mkdir -p "$(pdir snell)"
    local tmpzip
    mktemp_pd tmpzip
    download_https "$url" "$tmpzip" "Snell v${major} $ver" \
        || die "Snell v${major}下载失败: $url"
    extract_zip_binary "$tmpzip" snell-server "$(pbin snell)" "Snell v${major}" \
        || die "Snell v${major}安全解压失败"
    rm -f "$tmpzip"
    verify_download "$(pbin snell)" "Snell v${major}" 50000
    info "Snell v${major}下载完成"
}

snell_configure() {
    local port="$1" psk="$2" listen_addr="${3:-auto}" mode="${4:-}"
    local ipv6_enabled="false" mode_line=""
    ...
    if [ -n "$mode" ]; then
        case "$mode" in default|unshaped|unsafe-raw) ;; *) die "Snell v6 mode无效: $mode" ;; esac
        mode_line="mode = ${mode}"
    fi
...
snell_output() {
    local port="$1" psk="$2" major
    major=$(snell_installed_major 2>/dev/null || echo "${PD_OPT_SNELL_MAJOR:-5}")
    snell_validate_major "$major" || return 1
    local tls_pass="" tls_sni="" tls_proto="" tls_values="" mode="" mode_arg=""
    if [ "$major" = 6 ]; then
        mode=$(snell_current_mode)
        case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=$(state_get snell snell_mode 2>/dev/null || echo default) ;; esac
        case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=default ;; esac
        mode_arg=", mode=${mode}"
    fi
    tls_values=$(snell_shadowtls_unit_values 2>/dev/null || true)
    if [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && [ -z "$tls_values" ]; then
        err "ShadowTLS unit存在但凭据无法安全读取，拒绝生成Snell配置"
        return 1
    fi
    ...
    if [ -n "$tls_pass" ]; then
        output_header "Snell v${major} + ShadowTLS" "$port"
        echo -e "PSK:     ${GREEN}${psk}${RESET}"
        echo -e "TLS密码: ${GREEN}${tls_pass}${RESET}"
        echo -e "TLS SNI: ${GREEN}${tls_sni}${RESET}"
        echo -e "协议:    ${GREEN}ShadowTLS v${tls_proto}${RESET}"
        echo ""
        echo -e "${CYAN}[Surge 配置]${RESET}"
        echo -e "${GREEN}Proxy = snell, ${host4}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, shadow-tls-password=${tls_pass}, shadow-tls-sni=${tls_sni%:*}, shadow-tls-version=${tls_proto}${RESET}"
        [ -n "$host6" ] && echo -e "${GREEN}Proxy-IPv6 = snell, ${host6}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, shadow-tls-password=${tls_pass}, shadow-tls-sni=${tls_sni%:*}, shadow-tls-version=${tls_proto}${RESET}"
    else
        output_header "Snell v${major}" "$port"
        echo -e "PSK:    ${GREEN}${psk}${RESET}"
        echo ""
        echo -e "${CYAN}[Surge 配置]${RESET}"
        echo -e "${GREEN}Proxy = snell, ${host4}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, tfo=true${RESET}"
        [ -n "$host6" ] && echo -e "${GREEN}Proxy-IPv6 = snell, ${host6}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, tfo=true${RESET}"
    fi
    output_footer
}

repair_snell_ipv6() {
    local port psk major mode=""
    port=$(state_get snell "port")
    psk=$(conf_value "psk" "$(pdir snell)/snell.conf" || echo "")
    [ -n "$port" ] && [ -n "$psk" ] || return 0
    major=$(snell_installed_major 2>/dev/null || echo 5)
    if [ "$major" = 6 ]; then
        mode=$(snell_current_mode)
        case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=$(state_get snell snell_mode 2>/dev/null || echo default) ;; esac
        case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=default ;; esac
    ...
    local shadowtls_enabled=0
    [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && shadowtls_enabled=1
    prepare_snell_listen_family "$major" "$shadowtls_enabled"
    ...
    if [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ]; then
        ...
        [ "$major" != 6 ] || mode=unsafe-raw
        snell_configure "${server_arg##*:}" "$psk" "127.0.0.1" "$mode" || return 1
        write_shadowtls_unit "$port" "${server_arg##*:}" "$tls_sni" "$tls_pass" "$v3_arg" "$bin" || return 1
        systemctl daemon-reload || return 1
    else
        snell_configure "$port" "$psk" auto "$mode" || return 1
        systemctl daemon-reload || return 1
    fi
}
...
install_shadowtls() {
    ...
    local tmpbin
    mktemp_pd tmpbin
    download_https "$url" "$tmpbin" "ShadowTLS $ver" "ihciah/shadow-tls" "$ver" >&2 \
        || { rm -f "$tmpbin"; die "ShadowTLS 下载失败"; }
    mv "$tmpbin" "$bin"
    chmod 755 "$bin"
    verify_download "$bin" "ShadowTLS" 1000000 >&2
...
snell_shadowtls_configure() {
    ...
    case "$tls_version" in v3|3) tls_version="v3" ;; v2|2) tls_version="v2" ;; *) die "PD_SNELL_TLS_VERSION仅支持v3或v2，当前: $tls_version" ;; esac
    [ -n "$tls_pass" ] || tls_pass=$(rand_pass)
    PD_OPT_SNELL_TLS_PASS="$tls_pass"
    local v3_arg="--v3"
    [ "$tls_version" = "v2" ] && v3_arg=""
    step "配置ShadowTLS ${tls_version} (SNI: $sni) ..."
    local svc="shadowtls-snell" bin
    bin=$(install_shadowtls)
    write_shadowtls_unit "$ext_port" "$int_port" "${sni}:443" "$tls_pass" "$v3_arg" "$bin"
    systemctl daemon-reload
    systemctl enable "$svc" >/dev/null || { err "ShadowTLS enable失败"; return 1; }
    systemctl restart "$svc" || { err "ShadowTLS启动失败，查看journalctl -u $svc -n 20"; return 1; }
    info "ShadowTLS ${tls_version}已启动（公网$ext_port → Snell本机$int_port）"
}

snell_current_mode() {
    local mode
    mode=$(conf_value mode "$(pdir snell)/snell.conf" 2>/dev/null || true)
    case "$mode" in default|unshaped|unsafe-raw) printf '%s\n' "$mode" ;; *) printf '\n' ;; esac
}

snell_allocate_backend_port() {
    local port attempts=0
    while [ "$attempts" -lt 100 ]; do
        port=$(( (RANDOM % 15000) + 50000 ))
        if ! port_in_use "$port" && ! state_port_range_conflict "$port" "$port" snell; then
            printf '%s\n' "$port"
            return 0
        fi
        attempts=$((attempts + 1))
    done
    err "无法分配Snell内部端口（50000-65000）"
    return 1
}

snell_prepare_manage_network() {
    PD_LISTEN_FAMILY="$(state_get snell listen_family 2>/dev/null || true)"
    [ -n "$PD_LISTEN_FAMILY" ] || PD_LISTEN_FAMILY=auto
    prepare_listen_family
}

snell_enable_shadowtls() {
    state_installed snell || { err "Snell未安装"; return 1; }
    [ ! -f "$SYSTEMD_DIR/shadowtls-snell.service" ] || { warn "ShadowTLS已启用，请使用修改配置"; return 0; }
    snell_prepare_manage_network || return 1
    transaction_begin snell || { transaction_discard_unstarted; return 1; }
    transaction_run snell_enable_shadowtls_impl
}

snell_enable_shadowtls_impl() {
    local port psk major inner current_mode restore_mode="" runtime_mode=""
    port=$(state_get snell port); psk=$(conf_value psk "$(pdir snell)/snell.conf" || true)
    major=$(snell_installed_major)
    [ -n "$port" ] && [ -n "$psk" ] || die "Snell端口或PSK缺失"
    inner=$(snell_allocate_backend_port) || return 1
    current_mode=$(snell_current_mode)
    if [ "$major" = 6 ]; then
        case "$current_mode" in default|unshaped) restore_mode="$current_mode" ;; *) restore_mode="${PD_OPT_SNELL_V6_MODE:-default}" ;; esac
        runtime_mode="unsafe-raw"
    fi
    snell_configure "$inner" "$psk" 127.0.0.1 "$runtime_mode"
    systemctl restart snell || die "Snell切换本机后端监听失败"
    snell_shadowtls_configure "$port" "$inner"
    verify_shadowtls_stack "$port" "$inner" || die "ShadowTLS两层传输验证失败"
    state_set_fields snell \
        "shadowtls_enabled=1" "shadowtls_backend_port=$inner" \
        "shadowtls_sni=$PD_OPT_SNELL_TLS_SNI" "shadowtls_tls_version=${PD_OPT_SNELL_TLS_VERSION#v}" \
        "snell_mode=$runtime_mode" "snell_mode_before_shadowtls=$restore_mode"
    verify_and_record_listen_families snell "$port" tcp shadowtls-snell \
        || die "ShadowTLS公网监听地址族验证失败"
    info "已在不重装Snell的情况下启用ShadowTLS"
}

snell_disable_shadowtls() {
    state_installed snell || { err "Snell未安装"; return 1; }
    [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] || { warn "ShadowTLS未启用，当前已是普通Snell"; return 0; }
    snell_prepare_manage_network || return 1
    transaction_begin snell || { transaction_discard_unstarted; return 1; }
    transaction_run snell_disable_shadowtls_impl
}

snell_disable_shadowtls_impl() {
    local port psk major restore_mode="" unit="$SYSTEMD_DIR/shadowtls-snell.service"
    port=$(state_get snell port); psk=$(conf_value psk "$(pdir snell)/snell.conf" || true)
    major=$(snell_installed_major)
    [ -n "$port" ] && [ -n "$psk" ] || die "Snell端口或PSK缺失"
    if [ "$major" = 6 ]; then
        restore_mode=$(state_get snell snell_mode_before_shadowtls 2>/dev/null || true)
        case "$restore_mode" in default|unshaped) ;; *) restore_mode="default" ;; esac
    fi
    unit_is_pd_owned "$unit" || die "ShadowTLS unit所有权无法确认"
    systemctl disable --now shadowtls-snell >/dev/null || die "停止ShadowTLS失败"
    rm -f "$unit"
    [ ! -L /opt/shadowtls ] || die "拒绝删除ShadowTLS符号链接目录"
    [ -f /opt/shadowtls/.pd-proxy-owned ] || die "ShadowTLS目录所有权无法确认"
    rm -rf /opt/shadowtls
    if [ "$major" != 6 ]; then
        PD_LISTEN_FAMILY=ipv4
        prepare_listen_family
    fi
    snell_configure "$port" "$psk" auto "$restore_mode"
    systemctl daemon-reload
    systemctl restart snell || die "恢复普通Snell监听失败"
    verify_port "$port" snell tcp || die "普通Snell恢复验证失败"
    state_set_fields snell \
        "shadowtls_enabled=0" "shadowtls_backend_port=" "shadowtls_sni=" "shadowtls_tls_version=" \
        "snell_mode=$restore_mode" "snell_mode_before_shadowtls="
    verify_and_record_listen_families snell "$port" tcp snell \
        || die "普通Snell监听地址族验证失败"
    info "已关闭ShadowTLS并恢复普通Snell；Snell版本和PSK保持不变"
}

snell_reconfigure_shadowtls() {
    state_installed snell || { err "Snell未安装"; return 1; }
    [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] || { err "ShadowTLS未启用，请先启用"; return 1; }
    snell_prepare_manage_network || return 1
    transaction_begin snell || { transaction_discard_unstarted; return 1; }
    transaction_run snell_reconfigure_shadowtls_impl
}

snell_reconfigure_shadowtls_impl() {
    local port inner
    port=$(state_get snell port)
    inner=$(state_get snell shadowtls_backend_port 2>/dev/null || true)
    if ! [[ "$inner" =~ ^[0-9]+$ ]]; then
        inner=$(unit_arg_value --server "$SYSTEMD_DIR/shadowtls-snell.service" 2>/dev/null || true)
        inner="${inner##*:}"
    fi
    [[ "$port" =~ ^[0-9]+$ ]] && [[ "$inner" =~ ^[0-9]+$ ]] \
        || die "ShadowTLS端口状态缺失"
    snell_shadowtls_configure "$port" "$inner"
    verify_shadowtls_stack "$port" "$inner" || die "修改后的ShadowTLS验证失败"
    state_set_fields snell \
        "shadowtls_enabled=1" "shadowtls_backend_port=$inner" \
        "shadowtls_sni=$PD_OPT_SNELL_TLS_SNI" "shadowtls_tls_version=${PD_OPT_SNELL_TLS_VERSION#v}"
    info "ShadowTLS配置已更新，Snell未重装"
}
...
install_protocol() {
    local proto="$1" key cur_sts=false want_sts=false current_major requested_major
    assert_base_dir_safe || return 1
    key=$(pkey "$proto")
    if state_installed "$key"; then
        if [ "$proto" = snell ]; then
            current_major=$(snell_installed_major) || return 1
            if [ -n "${PD_SNELL_MAJOR:-}" ] || [ -n "${PD_SNELL_VERSION:-}" ]; then
                requested_major="${PD_OPT_SNELL_MAJOR:-$current_major}"
                [ -z "${PD_SNELL_VERSION:-}" ] || requested_major=$(snell_version_major "$PD_SNELL_VERSION")
                [ "$requested_major" = "$current_major" ] \
                    || { err "现有Snell为v${current_major}；安装命令不会覆盖切换协议大版本"; return 1; }
            fi
        ...
        warn "$(pname "$proto") 已安装，执行当前大版本最新版检查..."
...
install_protocol_impl() {
    ...
    # Snell协议大版本与ShadowTLS是两个独立维度。
    local display_name="$name"
    if [ "$proto" = snell ]; then
        snell_validate_major "$PD_OPT_SNELL_MAJOR" || return 1
        case "$PD_OPT_SNELL_MODE" in standard|shadowtls) ;; *) die "PD_SNELL_MODE仅支持standard或shadowtls" ;; esac
        case "$PD_OPT_SNELL_V6_MODE" in default|unshaped) ;; *) die "PD_SNELL_V6_MODE仅支持default或unshaped" ;; esac
        display_name="Snell v${PD_OPT_SNELL_MAJOR}"
        [ "$PD_OPT_SNELL_MODE" = shadowtls ] && display_name+=" + ShadowTLS"
    fi
    title "安装 $display_name"

    if [ "$proto" = snell ]; then
        local snell_shadowtls_enabled=0
        [ "$PD_OPT_SNELL_MODE" = shadowtls ] && snell_shadowtls_enabled=1
        prepare_snell_listen_family "$PD_OPT_SNELL_MAJOR" "$snell_shadowtls_enabled"
    else
        prepare_listen_family
    fi
    require_public_endpoint
    ...
    local ver=""
    if [ "$proto" = snell ]; then
        step "获取Snell v${PD_OPT_SNELL_MAJOR}版本..."
        ver=$(snell_get_version "$PD_OPT_SNELL_MAJOR") \
            || die "Snell v${PD_OPT_SNELL_MAJOR}版本检测失败，已停止安装"
        require_version "$ver" "Snell v${PD_OPT_SNELL_MAJOR}"
        info "版本: $ver"
        snell_download "$ver"
    ...
    step "写入配置..."
    local snell_runtime_mode="" snell_restore_mode=""
    if [ "$proto" = snell ] && [ "$PD_OPT_SNELL_MAJOR" = 6 ]; then
        snell_restore_mode="$PD_OPT_SNELL_V6_MODE"
        snell_runtime_mode="$snell_restore_mode"
        [ "$PD_OPT_SNELL_MODE" = shadowtls ] && snell_runtime_mode="unsafe-raw"
    fi
    if [ "$proto" = "snell" ] && [ "$PD_OPT_SNELL_MODE" = "shadowtls" ]; then
        # ShadowTLS模式：Snell仅监听本机内部端口，ShadowTLS接管原公网端口。
        ...
        while port_in_use "$snell_int"; do
            ...
            [ $_sni_attempts -lt 100 ] || die "无法分配Snell内部端口（50000-65000）"
        done
        snell_configure "$snell_int" "$pass" "127.0.0.1" "$snell_runtime_mode"
    elif [ "$proto" = snell ]; then
        snell_configure "$port" "$pass" auto "$snell_runtime_mode"
    else
    ...
    if [ "$proto" = snell ]; then
        state_set_fields snell \
            "snell_major=$PD_OPT_SNELL_MAJOR" \
            "shadowtls_enabled=$([ "$PD_OPT_SNELL_MODE" = shadowtls ] && echo 1 || echo 0)" \
            "shadowtls_backend_port=$([ "$PD_OPT_SNELL_MODE" = shadowtls ] && echo "$snell_int" || echo '')" \
            "snell_mode=$snell_runtime_mode" \
            "snell_mode_before_shadowtls=$snell_restore_mode"
    fi
...
upgrade_protocol_impl() {
    local proto="$1" key name svc bin bin_bak ver="" old_ver port hop transport major=""
    ...
    if [ "$proto" = snell ]; then
        major=$(snell_installed_major) || die "无法识别已安装Snell大版本"
        step "获取Snell v${major}最新版本..."
        ver=$(snell_get_version "$major") || die "Snell v${major}版本检测失败，已停止升级"
    else
        step "获取最新版本..."
    ...
    state_set "$key" version "$ver"
    [ "$proto" != snell ] || state_set "$key" snell_major "$major"
...
show_config_only() {
    ...
    case $proto in
        snell)
            local psk tls_pass tls_sni tls_proto major mode="" mode_arg=""
            psk=$(conf_value "psk" "$(pdir snell)/snell.conf" || echo "")
            [ -n "$psk" ] || { err "Snell PSK 缺失，拒绝生成配置"; return 1; }
            major=$(snell_installed_major) || { err "Snell大版本状态无效"; return 1; }
            if [ "$major" = 6 ]; then
                mode=$(snell_current_mode)
                case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=$(state_get snell snell_mode 2>/dev/null || echo default) ;; esac
                case "$mode" in default|unshaped|unsafe-raw) ;; *) mode=default ;; esac
            ...
            if [ -n "$tls_values" ]; then
                ...
                tls_proto=$(printf '%s\n' "$tls_values" | sed -n '3p')
                [ -n "$tls_proto" ] || tls_proto="2"
                echo "Proxy = snell, ${host4}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, shadow-tls-password=${tls_pass}, shadow-tls-sni=${tls_sni%:*}, shadow-tls-version=${tls_proto}"
                [ -n "$host6" ] && echo "Proxy-IPv6 = snell, ${host6}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, shadow-tls-password=${tls_pass}, shadow-tls-sni=${tls_sni%:*}, shadow-tls-version=${tls_proto}"
            else
                echo "Proxy = snell, ${host4}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, tfo=true"
                [ -n "$host6" ] && echo "Proxy-IPv6 = snell, ${host6}, ${port}, psk=${psk}, version=${major}${mode_arg}, reuse=true, tfo=true"
            fi ;;
        hy2)
...
show_status() {
    ...
    for proto in $ALL_PROTOS; do
        ...
        if state_installed "$(pkey "$proto")" && [ "$proto" = snell ]; then
            local snell_major
            snell_major=$(snell_installed_major 2>/dev/null || echo "?")
            name="Snell v${snell_major}"
            [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && name="Snell v${snell_major}+STS"
        fi
        if state_installed "$key"; then
            local port=$(state_get "$key" "port")
...
# ---- ShadowTLS交互配置 ----
menu_prompt_shadowtls_options() {
    local preserve="${1:-0}" current_values="" current_pass="" current_sni="" current_proto="3"
    local sni_c sni_custom tls_v pass_input
    if [ "$preserve" = 1 ]; then
        current_values=$(snell_shadowtls_unit_values 2>/dev/null || true)
        current_pass=$(printf '%s\n' "$current_values" | sed -n '1p')
        current_sni=$(printf '%s\n' "$current_values" | sed -n '2p'); current_sni="${current_sni%:*}"
        current_proto=$(printf '%s\n' "$current_values" | sed -n '3p')
        [ -n "$current_sni" ] || current_sni="www.microsoft.com"
        [ -n "$current_proto" ] || current_proto=3
    else
        current_sni="${PD_OPT_SNELL_TLS_SNI:-www.microsoft.com}"
    fi
    echo ""
    echo -e "  ${MAGENTA}▸ ShadowTLS伪装SNI${RESET}"
    echo "  [1] www.microsoft.com  [2] www.apple.com"
    echo "  [3] cloudflare.com      [4] 自定义"
    echo -ne "  选择 [保留/默认 ${current_sni}]: "
    read -r sni_c
    case "$sni_c" in
        1) PD_OPT_SNELL_TLS_SNI="www.microsoft.com" ;;
        2) PD_OPT_SNELL_TLS_SNI="www.apple.com" ;;
        3) PD_OPT_SNELL_TLS_SNI="cloudflare.com" ;;
        4) echo -ne "  SNI: "; read -r sni_custom; PD_OPT_SNELL_TLS_SNI="${sni_custom:-$current_sni}" ;;
        *) PD_OPT_SNELL_TLS_SNI="$current_sni" ;;
    esac
    echo -ne "  ShadowTLS版本 [3；输入2使用v2]: "
    read -r tls_v
    case "$tls_v" in 2) PD_OPT_SNELL_TLS_VERSION="v2" ;; "") PD_OPT_SNELL_TLS_VERSION="v${current_proto}" ;; *) PD_OPT_SNELL_TLS_VERSION="v3" ;; esac
    if [ "$preserve" = 1 ]; then
        echo -ne "  新密码（留空保持原密码）: "
        IFS= read -rs pass_input; echo ""
        PD_OPT_SNELL_TLS_PASS="${pass_input:-$current_pass}"
    else
        echo -ne "  自定义密码（留空自动生成）: "
        IFS= read -rs pass_input; echo ""
        PD_OPT_SNELL_TLS_PASS="$pass_input"
    fi
}
...
menu_snell_config() {
    ...
    while true; do
        echo ""
        echo -e "  ${MAGENTA}▸ 选择Snell协议版本${RESET}"
        echo -e "  ${CYAN}┌──────────────────────────────────────┐${RESET}"
        echo -e "  ${CYAN}│${RESET} [1] Snell v4                         ${CYAN}│${RESET}"
        echo -e "  ${CYAN}│${RESET} [2] ${GREEN}Snell v5${RESET}  稳定版                ${CYAN}│${RESET}"
        echo -e "  ${CYAN}│${RESET} [3] ${YELLOW}Snell v6${RESET}  当前为Beta            ${CYAN}│${RESET}"
        echo -e "  ${CYAN}│${RESET} [B] ${DIM}返回${RESET}                            ${CYAN}│${RESET}"
        echo -e "  ${CYAN}└──────────────────────────────────────┘${RESET}"
        echo -ne "  ${MAGENTA}▸${RESET} 选择 [2]: "
        read -r cc
        case "$cc" in
            1) PD_OPT_SNELL_MAJOR=4; break ;;
            3) PD_OPT_SNELL_MAJOR=6; PD_OPT_SNELL_V6_MODE=default; break ;;
            [Bb]) return 1 ;;
            [Qq]) info "再见 👋"; exit 0 ;;
            *) PD_OPT_SNELL_MAJOR=5; break ;;
        esac
    done
    echo ""
    echo -e "  ${MAGENTA}▸ Snell v${PD_OPT_SNELL_MAJOR}是否配置ShadowTLS${RESET}"
    echo "  [1] 普通Snell"
    echo "  [2] 配置ShadowTLS（后续可随时关闭）"
    echo -ne "  选择 [1]: "
    read -r cc
    case "$cc" in
        2) PD_OPT_SNELL_MODE=shadowtls; menu_prompt_shadowtls_options 0 ;;
        *) PD_OPT_SNELL_MODE=standard ;;
    esac
...
menu_install() {
    ...
    echo -e "  ${CYAN}┌──────────────────────────────────────┐${RESET}"
    echo -e "  ${CYAN}│${RESET} [1] ${GREEN}Snell${RESET}           ${DIM}v4 / v5 / v6${RESET}      ${CYAN}│${RESET}"
...
menu_protocol_manage_generic() {
    local proto="$1" cc
    while true; do
        echo ""
        echo -e "  ${MAGENTA}▸ $(pname "$proto")管理${RESET}"
        echo "  [1] 查看配置      [2] 升级"
        echo "  [3] 重启          [4] 停止"
        echo "  [5] 启动          [6] 日志"
        echo "  [7] 卸载          [B] 返回"
        echo -ne "  选择: "; read -r cc
        case "$cc" in
            1) show_config_only "$proto" ;;
            2) run_write_locked upgrade_protocol "$proto" ;;
            3) run_write_locked restart_service "$proto" ;;
            4) run_write_locked stop_service "$proto" ;;
            5) run_write_locked start_service "$proto" ;;
            6) show_log "$proto" ;;
            7) run_write_locked uninstall_protocol "$proto"; return 1 ;;
            [Bb]) return 1 ;;
            [Qq]) info "再见 👋"; exit 0 ;;
            *) warn "无效选择" ;;
        esac
...
menu_snell_manage() {
    local cc confirm major version port sts
    while true; do
        major=$(snell_installed_major 2>/dev/null || echo "?")
        version=$(state_get snell version 2>/dev/null || true)
        port=$(state_get snell port 2>/dev/null || true)
        sts="未启用"; [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && sts="已启用"
        echo ""
        echo -e "  ${MAGENTA}▸ Snell深入管理${RESET}"
        echo -e "  版本: ${GREEN}v${major} (${version:-未知})${RESET}  端口: ${port:-未知}  ShadowTLS: ${sts}"
        echo "  [1] 查看状态与配置    [2] 输出Surge配置"
        echo "  [3] 启用ShadowTLS     [4] 修改ShadowTLS"
        echo "  [5] 关闭ShadowTLS     [6] 更新当前大版本"
        echo "  [7] 重启              [8] 停止/启动"
        echo "  [9] 查看日志          [10] 卸载Snell"
        echo "  [B] 返回"
        echo -ne "  选择: "; read -r cc
        case "$cc" in
            1) show_status; show_config_only snell ;;
            2) show_config_only snell ;;
            3) menu_prompt_shadowtls_options 0; run_write_locked snell_enable_shadowtls ;;
            4) menu_prompt_shadowtls_options 1; run_write_locked snell_reconfigure_shadowtls ;;
            5)
                echo -ne "  确认关闭ShadowTLS并恢复普通Snell？输入yes: "; read -r confirm
                [ "$confirm" = yes ] && run_write_locked snell_disable_shadowtls || info "已取消" ;;
            6) run_write_locked upgrade_protocol snell ;;
            7) run_write_locked restart_service snell ;;
            8)
                if systemctl is-active --quiet snell 2>/dev/null; then
                    run_write_locked stop_service snell
                else
                    run_write_locked start_service snell
                fi ;;
            9) show_log snell ;;
            10) run_write_locked uninstall_protocol snell; return 1 ;;
            [Bb]) return 1 ;;
            [Qq]) info "再见 👋"; exit 0 ;;
            *) warn "无效选择" ;;
        esac
...
menu_manage() {
    local cc proto status name major
    echo -e "  ${MAGENTA}▸ 协议管理${RESET}"
    echo -e "  ${CYAN}┌────────────────────────────────────────────┐${RESET}"
    local i=0
    local -a protocols=()
    for proto in $ALL_PROTOS; do
        protocols+=("$proto"); i=$((i + 1)); name=$(pname "$proto")
        if state_installed "$(pkey "$proto")"; then
            status="${GREEN}已安装${RESET}"
            if [ "$proto" = snell ]; then
                major=$(snell_installed_major 2>/dev/null || echo "?")
                name="Snell v${major}"
                [ -f "$SYSTEMD_DIR/shadowtls-snell.service" ] && name+=" + ShadowTLS"
            fi
        else
            status="${DIM}未安装${RESET}"
        fi
        printf "  ${CYAN}│${RESET} [%s] %-23s %b ${CYAN}│${RESET}\n" "$i" "$name" "$status"
    done
    echo -e "  ${CYAN}│${RESET} [A] 查看全部配置   [E] 导出配置          ${CYAN}│${RESET}"
    echo -e "  ${CYAN}│${RESET} [B] 返回主菜单                           ${CYAN}│${RESET}"
    echo -e "  ${CYAN}└────────────────────────────────────────────┘${RESET}"
    echo -ne "  ${MAGENTA}▸${RESET} 选择: "; read -r cc
    case "$cc" in
        [1-4])
            proto="${protocols[$((cc - 1))]}"
            state_installed "$(pkey "$proto")" || { warn "$(pname "$proto")未安装，请先从安装协议进入"; return 0; }
            if [ "$proto" = snell ]; then
                menu_snell_manage || true
            else
                menu_protocol_manage_generic "$proto" || true
            fi ;;
        [Aa]) show_config ;;
        [Ee]) run_write_locked run_export ;;
        [Bb]) return 1 ;;
        [Qq]) info "再见 👋"; exit 0 ;;
        *) warn "无效选择" ;;
    esac
